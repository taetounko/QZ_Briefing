...
from qz_briefing.briefing import KiwoomCoreMarketCollector, normalize_price
...
class FakeMasterDataSource:
    ...
    def get_master_code_name(self, code: str) -> str:
        ...
        if code == self.fail_code:
            raise RuntimeError("master name unavailable")
        return {"005930": "삼성전자", "000660": "SK하이닉스"}[code]

    def get_master_last_price(self, code: str) -> str:
        self.calls.append(("price", code))
        return {"005930": "+72,500", "000660": "-185,000"}[code]
...
def test_collector_returns_samsung_and_sk_hynix_master_data() -> None:
    ...
    assert securities[0]["reference_price_raw"] == "+72,500"
    assert result["warnings"] == []


def test_one_security_failure_does_not_stop_the_other() -> None:
    source = FakeMasterDataSource(fail_code="005930")
    result = KiwoomCoreMarketCollector(source, clock=lambda: NOW).collect(context())
    securities = result["securities"]
    ...
    assert ("name", "000660") in source.calls
